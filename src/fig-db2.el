...
(defun fig//db2-handle-response (msg)
  ...
  (let* ((trimmed (s-chop-suffix "\r" (s-chop-suffix "\r\n" msg)))
    ...
    (cl-case sigil
      ...
      (?: (fig//db2-run-callback (string-to-number (seq-drop trimmed 1))))
...
(defun fig/db2-sadd (key &rest skeys)
  "Add SKEYS in set KEY in Redis."
  (fig/db2-cmd `("SADD" ,key ,@skeys) (lambda (_) nil)))

(defun fig/db2-srem (key &rest skeys)
  "Remove SKEYS from set KEY in Redis."
  (fig/db2-cmd `("SREM" ,key ,@skeys) (lambda (_) nil)))

(defun fig/db2-sismember (key skey k)
  "Check if SKEY is in set KEY in Redis and pass the resulting value to K."
  (fig/db2-cmd
   `("SISMEMBER" ,key ,skey)
   (lambda (x) (funcall k (not (= x 0))))))

(defun fig/db2-lpop (key k)
  "Left pop from list KEY from Redis and pass the popped value to K."
  (fig/db2-cmd `("LPOP" ,key) k))
